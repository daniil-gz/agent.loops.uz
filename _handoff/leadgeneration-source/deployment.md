...
## ILVI pilot publication — 2026-09-14
Published f341ffb to /cases/case-ilvi/ and pinned ILVI first on /leadgeneration/. New v2 story, supplied portrait cover, original brand logo, six-country grid, scoring route and exact-total ROAS infographic. Preview-first interaction works from rail and project list. User-selected display values $120 тыс. and $5 тыс. (no plus) labeled rounded; exact $118,758/$5,728 →20.73 remains in methodology. No monthly results or scoring thresholds published. No Meta calls.

13 renderer tests and Vite build passed; 1280/390/320 viewport checks, desktop/mobile preview and lower link→preview→full-case verified locally. Backup /root/backups/loops-www-20260914-185806.tar.gz. Dry-run and actual sync scoped to 11 public files and removal of two previous bundles. Public HTTP/1.1 readback: 11/11 byte-identical, private cases/_content/ilvi.json returns404 (evidence/ilvi/live-verification.json). CLI purge returned10000; no security or infrastructure changes made. IAB public full-case and landing fail with ERR_QUIC_PROTOCOL_ERROR; this is a browser transport limitation observed alongside successful public readback. Verified local copy opened for user. Original eight legacy full pages and NWL HTML unchanged.
